...
namespace WebBook.Payment
{
    public class VnPayService : IVnPayService
    {
        ...
        public string CreatePaymentUrl(Order model, HttpContext context)
        {
            ...
            //var tick = DateTime.Now.ToString("HHmmss");
            var pay = new VnPayLibrary();
            var urlCallBack = $"{context.Request.Scheme}:///{context.Request.Host.Value}{_configuration["PaymentCallBack:ReturnUrl"]}";

            pay.AddRequestData("vnp_Version", _configuration["Vnpay:Version"]);
            pay.AddRequestData("vnp_Command", _configuration["Vnpay:Command"]);
            pay.AddRequestData("vnp_TmnCode", _configuration["Vnpay:TmnCode"]);
            pay.AddRequestData("vnp_Amount", (string.Format("{0:0}", (model.TotalAmount + 30000) * 100)));
